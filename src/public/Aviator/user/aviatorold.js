...
const avatars = Array.from({ length: 72 }, (_, i) => `/Aviator/images/avtar/av-${i + 1}.png`);
...
function getRandomUserId() {
    // Generates a userid in the format [first character][***][last character]
    return `${getRandomAlphanumeric()}***${getRandomAlphanumeric()}`;
}
...
var randomDataArray = Array.from({ length: Math.floor(Math.random() * 39) + 10 }, () => ({
...
function info_data(intialData) {
    current_game_data = { id: Math.random() * 10000 };
    current_game_count = intialData.currentGameBetCount;
    show_bet_count(current_game_count);
    update_bet_list(randomDataArray, '#all_bets .mCSB_container', 1);
}
...
function cash_out_now(element, section_no, increment = '') {
    ...
    const currentBet = bet_array.find(val => val.section_no == section_no);
    currentBet.crashOut = incrementor;
    socket.emit('cashout', currentBet, (msg) => {
        console.log(msg,'cashoutmsg')
       if(msg){
        $("#wallet_balance").text(currency_symbol + (wallet_balance + parseInt(currentBet.bet_amount)*parseFloat(incrementor)).toFixed(2)); // Show Wallet Balance
        $("#header_wallet_balance").text(currency_symbol + (wallet_balance + parseInt(currentBet.bet_amount)*parseFloat(incrementor)).toFixed(2)); // Show Header Wallet Balance
       }
        if (section_no == 0) {
        ...
        if (section_no == 0) {
            ...
            $(".cashout-toaster1 .out-amount").html((parseFloat(currentBet.crashOut)*parseFloat(currentBet.bet_amount)).toFixed(2) + currency_symbol);
        ...
        if (section_no == 1) {
            ...
            $(".cashout-toaster2 .out-amount").html((parseFloat(currentBet.crashOut)*parseFloat(currentBet.bet_amount)).toFixed(2) + currency_symbol);
...
function incrementor(inc_no) {
    ...
    if(rand1<0.5){
        let index = Math.floor(Math.random() * randomDataArray.length);
        if (randomDataArray[index].cashout_multiplier === -1) {
            randomDataArray[index].cashout_multiplier = parseFloat(inc_no);
            update_bet_list(randomDataArray, '#all_bets .mCSB_container');
    ...
    $.ajax({
        ...
        success: function (result) {
            if (result.isSuccess && Object.keys(result.data).length > 0) {
                ...
                update_bet_list(randomDataArray, '#prev_bets .mCSB_container', 2);
                //khushbu
...
function bet_now(element, section_no) {
    ...
    if (stage_time_out == 1) {
        ...
        if (bet_amount >= min_bet_amount && bet_amount <= max_bet_amount) {
            ...
            let incrementor1 = $('#extra_incrementor').val();
            bet_array.push({ bet_type: bet_type, bet_amount: bet_amount, section_no: section_no, crash: incrementor1, phone: phone });
        }
    }
}
    ...
    if (bet_array.length > 0) {
        socket.emit('bet', bet_array, (msg) => {
            ...
            if ((msg[0] ? msg[0].status : true) && (msg[1] ? msg[1].status : true)) {
                ...
                $("#wallet_balance").text(currency_symbol + msg.at(-1).wallet.toFixed(2)); // Show Wallet Balance
                $("#header_wallet_balance").text(currency_symbol + msg.at(-1).wallet.toFixed(2)); // Show Header Wallet Balance
                wallet_balance = msg.at(-1).wallet;
                ...
                $(".error-toaster1 .msg").html("Insufficiant Balance");
