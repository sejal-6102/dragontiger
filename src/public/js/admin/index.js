...
function showJoinMember(data) {
    ...
    let result = '';
    const tbody = document.querySelector('.live_tbody');
    const newRow = tbody.insertRow();
        ...
        cell1.innerHTML =  `${((isNumber(bet)) ? bet : (bet == 'd') ? 'Red' : (bet == 'x') ? 'Green' : (bet == 't') ? 'Violet' : (bet == 'l') ? 'Big' : 'Small')}`
        cell2.innerHTML = money;  
        cell3.innerHTML = data.wallet; 
        cell4.innerHTML = data.win_wallet;
}
...
function showJoinMember2(data) {
    ...
    //     Join ${((isNumber(bet)) ? bet : (bet == 'd') ? 'Red' : (bet == 'x') ? 'Green' : (bet == 't') ? 'Violet' : (bet == 'l') ? 'Big' : 'Small')} ${money} ${data.win_wallet} ${data.wallet}
    //   </div>`;
    // result = result + 
    // `<div></div>`
