...
$(".0-9 .bet-num-line").click(function (e) {
  ...
  let info = $(this).attr("txt"); // Get the value from the clicked element
  let list = $("#result").attr("list-join"); // Get the list-join attribute value
  let checkClass = $(this).find(".bet-box-num").hasClass("active"); // Check if it has the 'active' class
  ...
  if (checkClass) {
    // If the element is already active, remove from array and list
    let newList = list.replace(info, ""); // Remove the value from the list-join string
    $("#result").attr("list-join", newList);

    // Remove the value from the array
    selectedValues = selectedValues.filter(value => value !== info);
  ...
  selectedValues.push(info);

  // Update the list-join attribute with the new value added
  ...
  // Log the array to see the current state
